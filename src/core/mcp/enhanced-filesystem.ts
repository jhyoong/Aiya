...
export class EnhancedFilesystemMCPClient extends FilesystemMCPClient {
  ...
  override async listTools(): Promise<Tool[]> {
    ...
    const enhancedTools: Tool[] = [
      ...
      },
      {
        ...
        description: 'Replace content using regex patterns with advanced options',
        inputSchema: {
          type: 'object',
          properties: {
            path: {
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            preview_first: {
              type: 'boolean',
              description: 'Preview all operations before executing (default: true)'
            }
          }
        }
      },
  ...
  override async callTool(name: string, args: Record<string, any>): Promise<ToolResult> {
    try {
      switch (name) {
        case 'preview_diff':
          return await this.previewDiff(args.path, args.content, args.show_line_numbers);
        case 'atomic_write':
          return await this.atomicWrite(args.path, args.content, args.create_backup);
        case 'atomic_edit':
          return await this.atomicEdit(args.path, args.old_content, args.new_content);
        case 'pattern_replace':
          return await this.patternReplace(args.path, args.pattern, args.replacement, args);
  ...
  private async atomicWrite(
    ...
    try {
      const result = await this.atomicOps.atomicWrite(filePath, content, { createBackup });
  ...
  private async atomicEdit(
    ...
    try {
      const result = await this.atomicOps.atomicEdit(filePath, oldContent, newContent);
  ...
  private async patternReplace(
    ...
    try {
      const validatedPath = await (this as any).security.validateFileAccess(filePath, 'read');
      const content = await (await import('fs/promises')).readFile(validatedPath, 'utf8');
      ...
      if (result.replacements > 0) {
        const atomicResult = await this.atomicOps.atomicWrite(filePath, result.newContent);
  ...
  private async executeQueue(previewFirst: boolean = true): Promise<ToolResult> {
    try {
      let previewText = '';
