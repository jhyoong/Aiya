...
export class CapabilityManager {
   ...
   */
  static getDefaultConfig(providerType: keyof ProviderModels, existingConfig?: Partial<ExtendedProviderConfig>): ExtendedProviderConfig {
   ...
   */
  static getCapabilities(providerType: keyof ProviderModels, modelName?: string): ProviderCapabilities {
    if (modelName) {
   ...
   */
  static getProviderDefaults(providerType: keyof ProviderModels): ProviderDefaults {
   ...
   */
  static getContextLengthOptions(providerType: keyof ProviderModels): Array<{label: string; value: number}> {
    const defaults = getProviderDefaults(providerType);
   ...
   */
  static getModelDescriptions(providerType: keyof ProviderModels): Record<string, string> {
    const models = this.getAvailableModels(providerType);
    const descriptions: Record<string, string> = {};
    
    models.forEach(modelName => {
      const metadata = getModelMetadata(providerType, modelName);
      descriptions[modelName] = metadata?.description || `${modelName} - No description available`;
    });
   ...
   */
  static getContextLengthInfo(providerType: keyof ProviderModels): Record<string, number> {
    const models = this.getAvailableModels(providerType);
   ...
   */
  static getCostInfo(providerType: keyof ProviderModels): Record<string, {input: number; output: number}> {
    const models = this.getAvailableModels(providerType);
   ...
   */
  static getApiKeyPrefix(providerType: keyof ProviderModels): string | undefined {
   ...
   */
  static validateApiKey(providerType: keyof ProviderModels, apiKey: string): boolean {
   ...
   */
  static getCapabilitiesDescription(providerType: keyof ProviderModels): string {
    const capabilities = {
   ...
   */
  static getThinkingOptions(providerType: keyof ProviderModels): Array<{label: string; value: boolean}> | null {
   ...
   */
  static getThinkingBudgetOptions(providerType: keyof ProviderModels): Array<{label: string; value: number}> | null {
    const capabilities = this.getCapabilities(providerType);
