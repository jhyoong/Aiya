...
export class ConfigurationGenerator {
   ...
   */
  private generateSingleProviderConfig(session: SetupSession): Partial<AiyaConfig> {
    return {
   ...
   */
  private generateMultiProviderConfig(session: SetupSession): Partial<AiyaConfig> {
    const providers: Record<string, ExtendedProviderConfig> = {};
    const usedNames = new Set<string>();
    
    // Add primary provider
    const primaryName = this.getProviderName(session.primaryProvider, usedNames);
   ...
   */
  private getProviderName(provider: ExtendedProviderConfig, existingNames: Set<string>): string {
    ...
    const baseName = cleanModel ? `${provider.type}-${cleanModel}` : provider.type;
   ...
   */
  private formatYAMLWithComments(config: Partial<AiyaConfig>, isMultiProvider: boolean): string {
...
`;
    ...
    if (isMultiProvider) {
      return baseComment + `# Multi-Provider Setup
...
`;
      ...
      return baseComment + `# Single Provider Setup
...
`;
   ...
   */
  private addInlineComments(yamlString: string, isMultiProvider: boolean): string {
