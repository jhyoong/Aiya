...
export class DiffGenerator {
  ...
  static formatDiff(
    ...
    const { showLineNumbers = true, colorOutput = true } = options;
  ...
  private static buildSimpleDiff(
    oldLines: string[],
    newLines: string[]
  ): DiffLine[] {
  ...
  private static groupIntoHunks(diffLines: DiffLine[]): DiffHunk[] {
    ...
    for (let i = 0; i < diffLines.length; i++) {
      ...
      if (line.type === 'added' || line.type === 'removed') {
        ...
        for (
          let j = i + 1;
          j < diffLines.length && diffLines[j]?.type === 'unchanged';
          j++
        ) {
          unchangedCount++;
        }
