...
describe('Shell Configuration Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    const security = new WorkspaceSecurity(workspaceRoot, ['.txt', '.js', '.ts'], 1024 * 1024);
  ...
  describe('Phase 5 Configuration Integration', () => {
    ...
    test('should validate confirmationThreshold on update', () => {
      ...
      }).toThrow('Invalid confirmationThreshold: 150. Must be between 0 and 100.');
    });
    ...
    test('should validate alwaysBlockPatterns on update', () => {
      ...
      }).toThrow('Invalid regex pattern in alwaysBlockPatterns[0]: [invalid regex');
    });
    ...
    test('should successfully update valid Phase 5 configuration', () => {
      ...
      expect(updatedConfig.alwaysBlockPatterns).toEqual(newConfig.alwaysBlockPatterns);
    });
    ...
    test('should validate Phase 5 configuration in CommandFilter directly', () => {
      ...
      }).toThrow('Invalid confirmationThreshold: 200. Must be between 0 and 100.');
    });
    ...
    test('should handle partial configuration updates for Phase 5 fields', () => {
      ...
      client.updateConfiguration({
        trustedCommands: ['^ls($|\\s)', '^pwd($|\\s)', '^echo($|\\s)', '^git status($|\\s)']
      });
    ...
    test('should validate regex patterns in Phase 5 configuration during integration', () => {
      ...
      }).toThrow('Invalid regex pattern in alwaysBlockPatterns[1]: [invalid regex pattern');
    });
  });
});