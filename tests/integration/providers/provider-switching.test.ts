...
describe('Provider Switching Integration Tests', () => {
  ...
  describe('Runtime Provider Switching', () => {
    test('should switch between providers mid-conversation', async () => {
      ...
      let response = await currentProvider.chat([
        { role: 'user', content: 'Hello, I\'m starting a conversation about AI' }
      ])
      conversationHistory.push(
        { role: 'user' as const, content: 'Hello, I\'m starting a conversation about AI' },
      ...
      response = await currentProvider.chat([
        ...conversationHistory,
        { role: 'user', content: 'Now I want to ask about vision capabilities' }
      ])
      conversationHistory.push(
        { role: 'user' as const, content: 'Now I want to ask about vision capabilities' },
        { role: 'assistant' as const, content: response.content }
      )
      ...
      response = await currentProvider.chat([
        ...conversationHistory,
        { role: 'user', content: 'Can you think through this complex problem step by step?' }
      ])
    ...
    test('should maintain capability awareness during switches', async () => {
      ...
      const visionRequest = [{ role: 'user' as const, content: 'Describe this [image]' }]
  ...
  describe('Failover Scenarios', () => {
    test('should handle primary provider failure with failover', async () => {
      const primaryProvider = MockProviderFactory.create(TEST_CONFIGS.openai.gpt4)
      const fallbackProvider = MockProviderFactory.create(TEST_CONFIGS.ollama.basic)
      ...
      // Should fail now
      await expect(primaryProvider.chat(message)).rejects.toThrow(/connection/i)
    ...
    test('should handle authentication failure with provider switch', async () => {
      ...
      const noAuthProvider = MockProviderFactory.create(TEST_CONFIGS.ollama.basic)

      const message = [{ role: 'user' as const, content: 'Test auth failover' }]

      // Simulate auth failure
      authProvider.simulateError('authentication')
      await expect(authProvider.chat(message)).rejects.toThrow(/authentication/i)
    ...
    test('should handle rate limiting with provider rotation', async () => {
      ...
      const message = [{ role: 'user' as const, content: 'Test rate limiting' }]
  ...
  describe('Load Balancing Simulation', () => {
    test('should distribute load across multiple providers', async () => {
      ...
      const loadDistribution = responses.reduce((acc, resp) => {
  ...
  describe('Configuration Switching', () => {
    test('should switch between different model configurations', async () => {
      ...
      const providers = configurations.map(config => MockProviderFactory.create(config))
      const message = [{ role: 'user' as const, content: 'Test different configurations' }]
    ...
    test('should handle capability-aware routing', async () => {
      ...
      const requests = [
        ...
        { content: 'Describe this [image]', needsVision: true, needsThinking: false },
        { content: 'Think through this complex problem', needsVision: false, needsThinking: true },
        { content: 'Analyze this [image] step by step', needsVision: true, needsThinking: true }
      ]
      ...
      for (const request of requests) {
        ...
        if (request.needsVision && request.needsThinking) {
          selectedProvider = providers.find(p => p.supportsVision() && p.supportsThinking())!
  ...
  describe('Session Management', () => {
    ...
    test('should handle provider cleanup and reinitialization', async () => {
      ...
      const response = await provider.chat([{ role: 'user', content: 'New session' }])
      assertValidProviderResponse(response)
      expect(provider.getMetrics().totalCalls).toBe(1)
    })
  })
