...
import {
  ...
  OPENAI_TEST_SCENARIOS,
} from '@tests/mocks/providers/mock-openai';
    ...
    describe('OpenAI-Specific Features', () => {
      ...
      test('should support vision capabilities', () => {
        ...
        if (provider.supportsVision()) {
          expect(['gpt-4o', 'gpt-4o-mini', 'gpt-4-turbo']).toContain(
      ...
      test('should handle quota exceeded', async () => {
        ...
        await expect(
          quotaProvider.chat([{ role: 'user', content: 'test' }])
        ).rejects.toThrow(/rate limit/i);
      });

      test('should support custom endpoints', () => {
        const customProvider = OPENAI_TEST_SCENARIOS.customEndpoint(
      ...
      test('should have accurate token counting', async () => {
        const provider = getProvider();
        const messages = [
          { role: 'user' as const, content: 'Test message for tokens' },
        ];
        const response = await provider.chat(messages);
      ...
      test('should support token-based streaming', async () => {
        ...
        const messages = [
          {
            role: 'user' as const,
            content: 'Stream this response token by token',
          },
        ];
        const chunks: any[] = [];
        ...
        contentChunks.forEach((chunk, index) => {
          ...
          if (index > 0) {
            expect(chunk.content.length).toBeGreaterThanOrEqual(
              contentChunks[index - 1].content.length
            );
          }
        });
      });
