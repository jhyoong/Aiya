...
import {
  ...
  GEMINI_TEST_SCENARIOS,
} from '@tests/mocks/providers/mock-gemini';
  ...
  protected testProviderSpecificFeatures(
    ...
    describe('Gemini-Specific Features', () => {
      ...
      test('should handle invalid API key', async () => {
        ...
        await expect(
          invalidProvider.chat([{ role: 'user', content: 'test' }])
      ...
      test('should stream with thinking content', async () => {
        if (getProvider().supportsThinking()) {
          const messages = [
            { role: 'user' as const, content: 'Think through this problem' },
          ];
          const chunks: any[] = [];
        ...
        const messages = [
          {
            role: 'user' as const,
            content:
              "Tell me about artificial intelligence. It's fascinating. How does it work?",
          },
        ];
        const chunks: any[] = [];
          ...
          const multimodalMessages = [
            {
              role: 'user' as const,
              content:
                'Describe this image and explain the code: [image] print("hello world")',
            },
          ];
      ...
      test('should handle thinking workflow correctly', async () => {
        ...
        const complexPrompt =
          'Solve this step by step: What is the optimal solution for this complex algorithmic problem?';

        const response = await thinkingProvider.chat([
      ...
      test('should support ultra-large contexts', async () => {
        ...
        const messages = [
          { role: 'user' as const, content: ultraLargeContent },
        ];
