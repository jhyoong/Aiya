...
describe('Provider Factory Tests', () => {
  ...
  describe('Provider Creation', () => {
    ...
    test('should throw error for unsupported provider types', () => {
      ...
      expect(() => MockProviderFactory.create(invalidConfig)).toThrow(
    ...
    test('should throw error for unimplemented providers', () => {
      ...
      expect(() => MockProviderFactory.create(anthropicConfig)).toThrow(
        /not implemented yet/
      );
    });
  });
  ...
  describe('Test Scenarios', () => {
    test('should create Ollama scenarios', async () => {
      const healthy = await MockProviderFactory.createScenario(
        'ollama',
        'healthy'
      );
      const offline = await MockProviderFactory.createScenario(
        'ollama',
        'offline'
      );
    ...
    test('should create OpenAI scenarios', async () => {
      ...
      const invalidKey = await MockProviderFactory.createScenario(
        'openai',
        'invalidApiKey'
      );
    ...
    test('should create Gemini scenarios', async () => {
      ...
      const thinking = await MockProviderFactory.createScenario(
        'gemini',
        'withThinking'
      );
    ...
    test('should handle custom scenario parameters', async () => {
      const customEndpoint = await MockProviderFactory.createScenario(
        'ollama',
        'customEndpoint',
        'http://test:11434'
      );
      ...
      const openaiCustom = await MockProviderFactory.createScenario(
        'openai',
        'customEndpoint',
    ...
    test('should throw error for unknown scenarios', async () => {
      await expect(
        MockProviderFactory.createScenario('ollama', 'unknownScenario')
      ).rejects.toThrow(/Unknown Ollama scenario/);

      await expect(
        MockProviderFactory.createScenario('openai', 'unknownScenario')
      ).rejects.toThrow(/Unknown OpenAI scenario/);

      await expect(
        MockProviderFactory.createScenario('gemini', 'unknownScenario')
      ).rejects.toThrow(/Unknown Gemini scenario/);
    });

    test('should throw error for unimplemented provider scenarios', async () => {
      await expect(
        MockProviderFactory.createScenario('anthropic', 'healthy')
  ...
  describe('Provider Capability Validation', () => {
    test('should create providers with correct capabilities', () => {
      const ollamaProvider = MockProviderFactory.create(
        TEST_CONFIGS.ollama.basic
      );
      const openaiProvider = MockProviderFactory.create(
        TEST_CONFIGS.openai.gpt4
      );
