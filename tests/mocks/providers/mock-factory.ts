...
export class MockProviderFactory {
   ...
   */
  static async createScenario(providerType: string, scenario: string, ...args: any[]): Promise<MockProvider> {
    switch (providerType) {
      case 'ollama':
      ...
      default:
        throw new Error(`Provider ${providerType} scenarios not implemented yet`)
    }
  }
...
export function createMockProvider(config: ExtendedProviderConfig): MockProvider {
  return MockProviderFactory.create(config)
}

export function createOllamaMock(overrides: Partial<ExtendedProviderConfig> = {}): MockOllamaProvider {
...
export const MockTestUtils = {
   ...
   */
  simulateNetworkIssues(providers: MockProvider[], errorType: 'connection' | 'timeout' = 'connection'): void {
  ...
  getCombinedMetrics(providers: MockProvider[]) {
    ...
    return {
      ...
      totalCalls: allMetrics.reduce((sum, metrics) => sum + metrics.totalCalls, 0),
      averageLatency: allMetrics.reduce((sum, metrics) => sum + metrics.averageLatency, 0) / allMetrics.length,
      overallErrorRate: allMetrics.reduce((sum, metrics) => sum + metrics.errorRate, 0) / allMetrics.length,
