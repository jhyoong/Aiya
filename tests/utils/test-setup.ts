...
expect.extend({
  ...
  toHaveProvider(received: any, provider: string) {
    ...
    return {
      message: () => `Expected configuration to have provider ${provider}, got ${received?.type}`,
      pass: hasProvider
    }
  },
  
  toMatchTokenCount(received: any, expected: { input: number; output: number }) {
    const hasValidTokens = received && 
    ...
    return {
      message: () => `Expected token count to match ${JSON.stringify(expected)}`,
      pass: hasValidTokens
    }
  }
})
