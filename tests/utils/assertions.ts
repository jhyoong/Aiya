 ...
 */
export function assertValidConfig(config: any): asserts config is ExtendedProviderConfig {
...
export function assertValidProviderResponse(response: any) {
  ...
  if (response.usage) {
    ...
    expect(response.usage.promptTokens || response.usage.input).toBeTypeOf('number')
    expect(response.usage.completionTokens || response.usage.output).toBeTypeOf('number')
  }
}
...
export function assertValidTokenUsage(usage: any) {
  ...
  const hasInputOutput = typeof usage.input === 'number' && typeof usage.output === 'number'
  const hasPromptCompletion = typeof usage.promptTokens === 'number' && typeof usage.completionTokens === 'number'
